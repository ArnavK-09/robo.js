...
<Callout type="info">Check out [Secrets](/docs/basics/secrets) for more</Callout>
...
```js filename="config/robo.mjs" showLineNumbers {3-5}
export default {
...
```js showLineNumbers /override/ {3-7}
export default {
...
<Callout>
	The `/dev` command is only up for grabs when your Robo's in development mode and chilling in your test server.
</Callout>
