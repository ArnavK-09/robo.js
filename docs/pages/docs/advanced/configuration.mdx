...
```js filename="config/robo.mjs" showLineNumbers
// @ts-check
...
```js /clientOptions/ filename="config/robo.mjs"
clientOptions: {
  intents: ['DirectMessages', 'Guilds', 'GuildMembers', 'GuildMessages', 'GuildVoiceStates', 'MessageContent'],
}
```
...
<Callout type="info">This feature works with directory and file prefixes, not glob patterns.</Callout>
...
```js /experimental/ filename="config/robo.mjs"
experimental: {
  buildDirectory: 'dist',
...
```js /sage/ filename="config/robo.mjs"
sage: {
  defer: true,
  deferBuffer: 2000,
  ephemeral: false,
