...
```js filename="/src/events/_start.js"
...
```js {4-6} filename="config/robo.mjs"
import { Intents } from 'discord.js'
...
<Callout type="info">Robo.js handles the `client.login()` part for you, so no worries there!</Callout>
