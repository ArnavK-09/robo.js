...
```js showLineNumbers filename="events/ping.js"
export default () => {
...
<FileTree>
	<FileTree.Folder name="src" open>
		<FileTree.Folder name="events" open>
			<FileTree.File name="ready.js" active />
			<FileTree.Folder name="ready" defaultOpen>
				<FileTree.File name="playStatus.js" active />
			</FileTree.Folder>
		</FileTree.Folder>
	</FileTree.Folder>
</FileTree>

```js showLineNumbers filename="src/events/ready/playStatus.js"
export default () => {
...
```js showLineNumbers filename="events/voiceStateUpdate.js" /newState/ /oldState/
export default (oldState, newState) => {
	// @TODO something with oldState and newState
}
```
...
Here's the lowdown: `_start` gets called _before_ your bot has logged in. You can return a Promise in these events and Robo.js will patiently wait for them to finish up, but don't leave it hanging too long – there's a 5-second time limit.
{/* [Check out the details on timeouts here](/docs/advanced/configuration#timeouts). */}
